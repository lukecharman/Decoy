...
final class RecorderTests: XCTestCase {
  ...
  func test_record_shouldRecordValidDecoy() {
    let recorder = Recorder()
    ...
    guard let stub = recording["stub"] as? [String: Any] else {
      return XCTFail(#function)
    }

    XCTAssertEqual(stub["json"] as? [String: String], ["A": "B"])
    XCTAssertEqual(stub["responseCode"] as? Int, 123)
  }
